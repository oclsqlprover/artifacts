...
; Example #3
; SQL: SELECT age >= 18 FROM Student WHERE Student_id = self
; sel1 = SELECT age >= 18 FROM Student WHERE Student_id = self
; expr1 = Student_id = self
; expr2 = Student_id
; expr3 = self
; expr4 = age >= 18
; expr5 = age
...
(declare-fun val-sel1-expr2 (Int) Int)
(declare-fun val-sel1-expr3 (Int) Int)
...
(assert (forall ((x Int))
    (=> (index-sel1 x)
        (= (= (val-sel1-expr1 x) TRUE)
           (and (not (= (val-sel1-expr2 x) nullInt))
                (not (= (val-sel1-expr3 x) nullInt))
...
(assert (forall ((x Int))
    (=> (index-sel1 x)
        (= (= (val-sel1-expr1 x) NULL)
           (or (= (val-sel1-expr2 x) nullInt)
               (= (val-sel1-expr3 x) nullInt))))))
(assert (forall ((x Int))
    (=> (index-sel1 x)
        (= (val-sel1-expr2 x) (val-student-age x)))))
(assert (forall ((x Int))
    (=> (index-sel1 x)
        (= (val-sel1-expr3 x) 18))))
...
; Subgoal C1: Prove that the following SQL execution always returns 1 row.
