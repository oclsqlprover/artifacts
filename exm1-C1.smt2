...
; sel = SELECT TRUE
; expr1 = TRUE
(declare-fun index-sel (Int) Bool)
(declare-fun val-sel-expr1 (Int) BOOL)

(assert (exists ((x Int))
    (and (index-sel x)
         (forall ((y Int))
             (=> (not (= x y)) (not (index-sel y)))))))
(assert (forall ((x Int))
    (=> (index-sel x) (= (val-sel-expr1 x) TRUE))))

; Subgoal C1: Prove that the following SQL execution always returns 1 row.
; E(x)(index-sel(x) /\ A(y)(~(y=x) => ~(index-sel(y)))
